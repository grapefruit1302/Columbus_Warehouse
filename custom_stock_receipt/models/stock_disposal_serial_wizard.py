...
class StockDisposalSerialWizard(models.TransientModel):
    ...
    selected_line_id = fields.Many2one('stock.receipt.disposal.line', 'Позиція',
                                     required=True, domain="[('disposal_id', '=', disposal_id), ('tracking_serial', '=', True)]")
    ...
    def default_get(self, fields_list):
        ...
        disposal_id = self.env.context.get('default_disposal_id')
        line_id = self.env.context.get('default_selected_line_id')
        warning_message = self.env.context.get('warning_message')

        if disposal_id and line_id:
            res['disposal_id'] = disposal_id
    ...
    def action_load_from_file(self):
        ...
        return {
            ...
            'res_model': 'stock.disposal.serial.import.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def _validate_serial_numbers(self):
        ...
        if duplicates:
            ...
            return {
                'type': 'ir.actions.act_window',
                'res_model': 'stock.disposal.serial.duplicate.wizard',
                'view_mode': 'form',
                'target': 'new',
                'context': {
                    'default_parent_wizard_id': self.id,
...
class StockDisposalSerialWizardLine(models.TransientModel):
    _name = 'stock.disposal.serial.wizard.line'
    ...
    line_id = fields.Many2one('stock.receipt.disposal.line', 'Позиція', required=True)
...
class StockDisposalSerialWizardSerial(models.TransientModel):
...
class StockDisposalSerialDuplicateWizard(models.TransientModel):
    _name = 'stock.disposal.serial.duplicate.wizard'
    _description = 'Обробка дублікатів серійних номерів'
    
    parent_wizard_id = fields.Many2one('stock.disposal.serial.wizard', 'Основний wizard', required=True)
...
class StockDisposalSerialImportWizard(models.TransientModel):
    _name = 'stock.disposal.serial.import.wizard'
    _description = 'Імпорт серійних номерів'
    
    parent_wizard_id = fields.Many2one('stock.disposal.serial.wizard', 'Основний wizard')
    selected_line_id = fields.Many2one('stock.receipt.disposal.line', 'Обрана позиція')
